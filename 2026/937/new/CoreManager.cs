...
public class CoreManager
{
    ...
    private async Task<Process?> RunProcessNormal(string fileName, CoreInfo? coreInfo, string configPath, bool displayLog)
    {
        Process proc = new()
        {
            ...
            {
                FileName = fileName,
                Arguments = string.Format(coreInfo.Arguments, (coreInfo.CoreType is ECoreType.brook or ECoreType.gost or ECoreType.chisel or ECoreType.ssh or ECoreType.sshpass or ECoreType.easytier or ECoreType.cloudflared or ECoreType.wstunnel or ECoreType.sudoku or ECoreType.usque) ? System.IO.File.ReadAllText(Utils.GetBinConfigPath(configPath)) :(coreInfo.AbsolutePath ? Utils.GetBinConfigPath(configPath).AppendQuotes() : configPath)),
